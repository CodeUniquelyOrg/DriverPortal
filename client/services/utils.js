...
// RegEx statement for Mobile Detection

/* eslint-disable */
const mobileCheck1 = /(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino/i;
...
export const checkIfMobile = () => {
  ...
  const agent = window.navigator.userAgent || window.navigator.vendor || window.opera;
  if (mobileCheck1.test(agent)) {
    isMobileDevice = true;
  }
  const token = agent.substr(0, 4);
  if (mobileCheck2.test(token)) {
    isMobileDevice = true;
  }
...
// his is useful to correct for floating point precision issues, less useful for integers.
...
export const average = (data :number[] = []) => {
  ...
  return (sigma && sigma / data.length) || 0;
};
